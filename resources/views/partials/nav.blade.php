...
<nav>
  <div class="nav-logo"><a href="{{ route('home') }}"><img src="{{ asset('assets/logos/convictos-branco.png') }}" alt="Convictos"></a></div>
  <div class="nav-links">
    <a href="{{ route('home') }}#sobre">Sobre</a>
    <a href="{{ route('home') }}#missao">Missão</a>
    <a href="{{ route('store.index') }}" class="{{ request()->routeIs('store.*') ? 'active' : '' }}">Produtos</a>
    <a href="{{ route('home') }}#valores">Valores</a>
    <a href="{{ route('cart.index') }}" class="nav-cart">
      Carrinho
      @if($cart->count() > 0)<span class="cart-badge">{{ $cart->count() }}</span>@endif
    </a>
...
<div class="mobile-menu" id="mobileMenu">
  ...
  <a href="{{ route('store.index') }}" onclick="document.getElementById('mobileMenu').classList.remove('open')">Produtos</a>
  <a href="{{ route('cart.index') }}" onclick="document.getElementById('mobileMenu').classList.remove('open')">Carrinho ({{ $cart->count() }})</a>
